...
while (($rowyear = $resultyear->fetchRow())!= false) {
		...
		$stmta = \OCP\DB::prepare('SELECT DISTINCT  MONTH(date) as month ,MONTHNAME(date) as monthname FROM `*PREFIX*facefinder_exif_module` inner  join oc_facefinder on (*PREFIX*facefinder_exif_module.photo_id= *PREFIX*facefinder.photo_id) Where uid_owner LIKE ? AND YEAR(date) = ? order by   date DESC');
